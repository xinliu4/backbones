...
old_halos = [line for line in formation_times if line[formation_time_index] > 2.]
sorted_oldhalos_index = sorted(range(len(old_halos)), key=lambda i: old_halos[i][1])
sorted_oldhalos = np.array([old_halos[index][0] for index in sorted_oldhalos_index])

print sorted_oldhalos[:10]
print sorted_oldhalos[-10:]
...
print "large"
for elem in sorted_oldhalos[-10:]:
	print hid_formation_dict[elem]
print "small"
for elem in sorted_oldhalos[:10]:
	print hid_formation_dict[elem]

smallhalo_found = 0
with open("small_old_halo_backbone.txt", "w") as smallhalo_f, open("maximum_mass_backbone.txt", "w") as maxmass_f:
	for backbone_file_i in range(subfiles_count):
		...
		for backbone in backbones:
			if(backbone[-1][1] != 499):
				...
				print backbone
				exit()
			if(backbone[-1][3] >= min_z0_m200_count):
				#if(backbone[-1][3]==maximum_z0_mass):
				if(backbone[-1][0] in sorted_oldhalos[-10:]):
					#print backbone[-1][3]
					#print backbone
					for line in backbone:
						if (line[4] != -1):
							maxmass_f.write("%d\t%d\t%d\t%f\n" %(line[0], line[1], line[3], line[4]))
				#if((backbone[-1][3] <= 4000 and hid_formation_dict[backbone[-1][0]]>=2. and smallhalo_found == 0)):
				if(backbone[-1][0] in sorted_oldhalos[:10]):
					print "high mass"
					print hid_formation_dict[backbone[-1][0]]
					print backbone[-1][3]
					print backbone
					smallhalo_found = 1
					for line in backbone:
						if(line[4] != -1):
							smallhalo_f.write("%d\t%d\t%d\t%f\n" %(line[0], line[1], line[3], line[4]))
